...
export function usePrintWizardExport({
  ...
  const downloadWithProject = async (quality: "standard" | "high") => {
    ...
    if (!activeBg) {
      showToast("먼저 AI 배경을 생성하거나 이미지를 업로드해 주세요.", "info");
      return;
    }
    setBusy(true);
    try {
      ...
      await downloadAndRemember({
        imageBlob,
        project,
        baseName: `print-smart-${quality}`,
        imageExt: "png",
        successMessage:
          quality === "high"
            ? "고화질 파일 + 보안 수정파일(.sca)을 저장하고 최근 목록에 등록했습니다."
            : "일반화질 파일 + 보안 수정파일(.sca)을 저장하고 최근 목록에 등록했습니다.",
      });
    } catch {
